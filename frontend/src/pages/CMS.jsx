...
const ProductPage = () => {
  ...
  const handleEdit = (prod) => {
    ...
    setFormData({ 
      ...
      packing_options: prod.packing_options || [{ product: '', packing: '', container: '', file: null }],
      type_options: prod.type_options || [{ name: '', description: '', file: null }],
        ...
        <table className="master-table">
          ...
          <tbody>
            {(products || []).map(prod => (
              <tr key={prod.id}>
                <td>
                  {prod.image ? <img src={prod.image} alt={prod.name} style={{ width: '40px', height: '40px', borderRadius: '4px', objectFit: 'cover' }} /> : <div style={{ width: '40px', height: '40px', backgroundColor: '#f3f4f6', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Box size={20} style={{ color: '#9ca3af' }} /></div>}
                </td>
                        ...
                        <div className="upload-zone row-upload-zone" onClick={() => document.getElementById(`type-file-${index}`).click()}>
                          {opt.file ? (
                            <div className="file-info compact">
                              <FileText size={18} color="#3943c7" />
                              <div className="file-details">
                                <span>Variation File Attached</span>
                              </div>
                          ...
                          <input id={`type-file-${index}`} type="file" hidden onChange={(e) => {
                            ...
                            if (file) {
                              const reader = new FileReader();
                              reader.onloadend = () => updateTypeRow(index, 'file', reader.result);
                              reader.readAsDataURL(file);
                            }
                          }} />
                        </div>
                  ...
                  <div className="form-group-light">
                    ...
                    <div className="upload-zone" onClick={() => document.getElementById('nutrition-upload').click()}>
                      {formData.nutrition_file ? (
                        <div className="file-info">
                          {formData.nutrition_file.startsWith('data:image') ? (
                            <img src={formData.nutrition_file} alt="Nutrition Preview" style={{ maxHeight: '150px', borderRadius: '8px' }} />
                          ) : (
                            <FileText size={32} color="#3943c7" />
                          )}
                          <div className="file-details">
                      ...
                      <input id="nutrition-upload" type="file" hidden onChange={(e) => {
                        ...
                        if (file) {
                          const reader = new FileReader();
                          reader.onloadend = () => setFormData({ ...formData, nutrition_file: reader.result });
                          reader.readAsDataURL(file);
                        }
                      }} accept="image/*,.pdf" />
                    </div>
                      ...
                      <div className="form-group-light" style={{ marginTop: '15px' }}>
                        ...
                        <div className="upload-zone row-upload-zone" onClick={() => document.getElementById(`row-file-${index}`).click()}>
                          ...
                          <input id={`row-file-${index}`} type="file" hidden onChange={(e) => {
                            ...
                            if (file) {
                              const reader = new FileReader();
                              reader.onloadend = () => updatePackingRow(index, 'file', reader.result);
                              reader.readAsDataURL(file);
                            }
                          }} />
                        </div>
